...
switch ($_POST['submit']) {
    ...
    if (isset($_POST["quatation_ID"])) {
      ...
      if ($_POST["approver_code"] != '') {
        ...
        $sql = "UPDATE quatation SET 
                work_process_status_user = 'success',
                work_process_status_approvepu = 'pending',
                status = 2, 
                approver_pu_code='$approver_code',
                approver_pu_nameTH='$approver_nameTH',
                comment_approver_user = '$comment_approver_user' , 
                date_time_stamp_approver_user = GETDATE(),
                date_time_stamp_update = GETDATE() 
        WHERE quatation_ID = '$quatation_ID'";

        if($status_gm == 'wait' || $status_gm == 'unsuccess'){
          $sql = "UPDATE quatation SET 
                  work_process_status_user = 'success',
                  work_process_status_gm = 'pending',
                  status = 2, 
                  approver_gm_code='$approver_code',
                  approver_gm_nameTH='$approver_nameTH',
                  comment_approver_user = '$comment_approver_user' , 
                  date_time_stamp_approver_user = GETDATE(),
                  date_time_stamp_update = GETDATE() 
                WHERE quatation_ID = '$quatation_ID'";
        }else if($status_md == 'wait' || $status_md == 'unsuccess'){
          $sql = "UPDATE quatation SET 
                  work_process_status_user = 'success',
                  work_process_status_md = 'pending',
                  status = 2, 
                  approver_md_code='$approver_code',
                  approver_md_nameTH='$approver_nameTH',
                  comment_approver_user = '$comment_approver_user' , 
                  date_time_stamp_approver_user = GETDATE(),
                  date_time_stamp_update = GETDATE() 
                WHERE quatation_ID = '$quatation_ID'";
          ...
          $message = '<html> 
                                    ...
                                    <FONT Size = "3"> 
                                        ...
                                        <table border="1" cellpadding="0" cellspacing="0" width="100%" style="font-family:Tahoma;font-size:16px">
                                                    ...
                                                    <tbody>
                                                        <td >Approved by Department Manager (ผจก.ของผู้ร้องขอ อนุมัติแล้ว)</td>
                                                        <td >' . $name_request . '</td>
                                                        <td >' . $email . '</td>
                                                        <td >' . $department . '</td>
                                                        <td >' . $tel . '</td>
  ...
  case 'cancel':
    ...
    $sql = "UPDATE quatation 
            SET work_process_status_user = 'unsuccess', 
            status = 6 , 
            comment_approver_user = '$comment_approver_user' , 
            date_time_stamp_approver_user = GETDATE(), 
            date_time_stamp_update = GETDATE()  
    ...
    if ($query1) {
      ...
      $message = '<html> 
                                ...
                                <FONT Size = "3"> 
                                    ...
                                    &nbsp;&nbsp; เนื่องจาก ' . $approver_user_nameTH . ' ได้ทำการกด <font color="red">ไม่อนุมัติ</font> <b>เอกสารเลขที่ : ' . $num_req . '</b><br> 
                                    ...
                                    <table border="1" cellpadding="0" cellspacing="0" width="100%" style="font-family:Tahoma;font-size:16px">
                                                ...
                                                <tbody>
                                                    <td >Disapproval by Department Manager (ให้ User Revise ใหม่)</td>
                                                    <td >' . $name_request . '</td>
                                                    <td >' . $email . '</td>
                                                    <td >' . $department . '</td>
                                                    <td >' . $tel . '</td>
